...
- **Watch Public Link Submissions**: `submission_created` (new and updated submissions) and `submission_abandoned` events for public-link submissions, with required 12-hour, 1-day, 3-day, or 1-week idle windows for abandoned submissions. A completed request never reaches it (one channel, one event)
